...
def _fingerprint_payload_from_cfg(cfg) -> Dict[str, Any]:
    # ✅ SSOT: model_spec 우선, 없으면 backbone에서 가져오되 "있을 때만"
    ms = getattr(cfg, "model_spec", None)

    def _get_int(obj, name: str, default: Optional[int] = None) -> int:
        if obj is None:
            if default is None:
    ...
    dim = _get_int(ms, "dim", getattr(getattr(cfg, "backbone", None), "dim", 64))
    bias = _get_int(ms, "bias", getattr(getattr(cfg, "backbone", None), "bias", 1))
    volterra_rank = _get_int(ms, "volterra_rank", getattr(getattr(cfg, "backbone", None), "volterra_rank", 2))
    ...
    payload = {
        ...
        "bias": bias,
        "volterra_rank": volterra_rank,
        "actions.order": list(ACTIONS_ORDER),
    }
    return payload
...
class FolderPairsDataset(Dataset):
    def __init__(self, input_dir: str, gt_dir: str, patch: int = 256, repeat: int = 1, strict: bool = True):
        ...
        gt_map: Dict[str, str] = {}
        for p in self.gt_paths:
            gt_map[make_key(p, self.gt_dir)] = p
        ...
        print(f"[FolderPairsDataset] matched={len(pairs)} miss={miss} input_dir={self.input_dir}")
        if strict and miss > 0:
            raise RuntimeError(f"[StrictPairing] missing pairs: miss={miss} in {self.input_dir}")
...
def build_train_dataset_from_cfg(cfg, *, strict_pairing: bool = True) -> Tuple[Dataset, List[Dict[str, Any]]]:
    ...
    # ✅ SSOT: patch 우선은 model_spec
    ms = getattr(cfg, "model_spec", None)
    patch = int(getattr(ms, "patch", None) or getattr(getattr(cfg, "backbone", None), "patch", 256))
    repeat_each = int(getattr(cfg, "repeat_each", 1))
    ...
    for sd in specs:
        ...
        inp = _join(data_root, str(sd["input_dir"]))
        gt = _join(data_root, str(sd["gt_dir"]))
        sub.append(FolderPairsDataset(inp, gt, patch=patch, repeat=repeat_each, strict=strict_pairing))
        resolved_specs.append({"name": name, "type": "folder", "input_dir": inp, "gt_dir": gt})
...
def dump_pair_sanity(loader: DataLoader, out_dir: str, n: int = 20):
    ...
    for batch in loader:
        ...
        for i in range(b):
            ...
            outp = os.path.join(out_dir, f"sanity_{cnt:03d}.png")
            pred_dummy = torch.zeros_like(lq_i)
            save_triplet_with_text(lq_i, pred_dummy, gt_i, outp, text_lines=["SANITY DUMP", ""])
            cnt += 1
            if cnt >= n:
                print(f"[SanityDump] saved {cnt} images -> {out_dir}")
                return
...
def _build_vetnet_model(in_chans: int, dim: int, bias: bool, volterra_rank: int, device: torch.device):
    ...
    try:
        ...
        m = VETNet(in_channels=in_chans, out_channels=in_chans, dim=dim, bias=bias, volterra_rank=volterra_rank).to(device)
...
def _find_latest_ckpt(save_dir: str) -> Optional[str]:
    ...
    for f in os.listdir(save_dir):
        if f.lower().endswith(".pth") and f.lower().startswith("epoch_"):
            cands.append(os.path.join(save_dir, f))
    if not cands:
        return None
    ...
    cands.sort(key=lambda p: epoch_of(p))
    return cands[-1]
...
def _load_resume(
    ...
    ckpt = torch.load(resume_path, map_location=str(device))
    if not isinstance(ckpt, dict) or "state_dict" not in ckpt:
        raise RuntimeError(f"[Resume] invalid checkpoint format: {resume_path}")

    model.load_state_dict(ckpt["state_dict"], strict=False)
    print(f"[Resume] loaded model: {resume_path} (strict=False)")
    ...
    if optimizer is not None and isinstance(ckpt.get("optimizer", None), dict):
        try:
            ...
            print(f"[Resume] WARN: optimizer load failed: {repr(e)}")
    ...
    if scaler is not None and isinstance(ckpt.get("scaler", None), dict):
        try:
            ...
            print(f"[Resume] WARN: scaler load failed: {repr(e)}")
...
def main():
    ...
    ms = getattr(cfg, "model_spec", None)
    patch = int(getattr(ms, "patch", None) or getattr(getattr(cfg, "backbone", None), "patch", 256))
    in_chans = int(getattr(ms, "in_chans", None) or getattr(getattr(cfg, "backbone", None), "in_chans", 3))
    dim = int(getattr(ms, "dim", None) or getattr(getattr(cfg, "backbone", None), "dim", 64))
    bias = bool(int(getattr(ms, "bias", None) or getattr(getattr(cfg, "backbone", None), "bias", 1)))
    volterra_rank = int(getattr(ms, "volterra_rank", None) or getattr(getattr(cfg, "backbone", None), "volterra_rank", 2))

    save_dir = args.save_dir or (cfg.ckpt["backbone_dir"] if isinstance(cfg.ckpt, dict) and "backbone_dir" in cfg.ckpt else "E:/VETAgent/checkpoints/backbone")
    os.makedirs(save_dir, exist_ok=True)
    ...
    print(f"[Device] {device} | amp={use_amp} channels_last={channels_last} tf32={tf32} | skimage={USE_SKIMAGE}")
    print(f"[SSOT] dim={dim} bias={int(bias)} volterra_rank={volterra_rank} in_chans={in_chans} patch={patch}")
    print(f"[Paths] save_dir={os.path.normpath(save_dir)} results_dir={os.path.normpath(results_dir)}")

    model = _build_vetnet_model(in_chans=in_chans, dim=dim, bias=bias, volterra_rank=volterra_rank, device=device)

    if channels_last and device.type == "cuda":
        model = model.to(memory_format=torch.channels_last)
        torch.backends.cudnn.benchmark = True

    train_set, _resolved = build_train_dataset_from_cfg(cfg, strict_pairing=True)
    loader = DataLoader(
        train_set,
        batch_size=int(args.batch_size),
        shuffle=True,
    ...
    if int(args.sanity_only) == 1:
        dump_pair_sanity(loader, out_dir=os.path.join(results_dir, "sanity"), n=int(args.sanity_dump))
        return
    ...
    # fingerprint just for logging (optional)
    fp_payload = _fingerprint_payload_from_cfg(cfg)
    backbone_fp = _sha256_of_json(fp_payload)
    print(f"[FP] {backbone_fp}")

    start_epoch = 1
    global_step = 0

    if int(args.resume) == 1:
        resume_path = args.resume_ckpt.strip() if args.resume_ckpt else _find_latest_ckpt(save_dir)
        if resume_path and os.path.exists(resume_path):
            info = _load_resume(resume_path, model, opt, scaler, device)
    ...
    if start_epoch > epochs:
        print(f"[Resume] start_epoch({start_epoch}) > target_epochs({epochs}). Nothing to do.")
        return

    steps_per_epoch = len(loader)
    print(f"[Train] steps_per_epoch={steps_per_epoch} | epochs_target={epochs} | start_epoch={start_epoch}")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for it, batch in enumerate(pbar, start=1):
            ...
            if USE_SKIMAGE and int(args.metric_every) > 0 and (global_step % int(args.metric_every) == 0 or it == steps_per_epoch):
                ps, ss = compute_psnr_ssim_skimage(pred_c.detach(), gt.detach(), max_images=int(args.metric_images_per_batch))
                metric_psnr_sum += ps
                metric_ssim_sum += ss
                metric_cnt += 1
            ...
            if int(args.iter_save_interval) > 0 and (global_step % int(args.iter_save_interval) == 0):
                with torch.no_grad():
                    ...
                    if USE_SKIMAGE:
                        p_one, s_one = compute_psnr_ssim_skimage(pr0.unsqueeze(0), gt0.unsqueeze(0), max_images=1)
                        line1 = f"PSNR={p_one:.2f}  SSIM={s_one:.4f}"
                    else:
                        line1 = "PSNR/SSIM: skimage OFF"

                    outp = os.path.join(results_dir, "iter", f"epoch_{epoch:03d}_gs_{global_step:08d}.png")
                    save_triplet_with_text(lq0, pr0, gt0, outp, text_lines=[line1, f"epoch={epoch} step={global_step}"])
        ...
        print(
            f"\n[Epoch {epoch:03d}] Loss={epoch_loss:.4f} PSNR={epoch_psnr:.2f} SSIM={epoch_ssim:.4f} time={_format_time(time.time() - t0)}"
        )
        ...
        ckpt = {
            ...
            "fingerprint": backbone_fp,
            "fingerprint_payload": fp_payload,
            "ssot": {"dim": dim, "bias": int(bias), "volterra_rank": volterra_rank, "in_chans": in_chans, "patch": patch},
        }
        out_name = f"epoch_{epoch:03d}_L{epoch_loss:.4f}_P{epoch_psnr:.2f}_S{epoch_ssim:.4f}.pth"
        out_path = os.path.join(save_dir, out_name)
        torch.save(ckpt, out_path)
...
"""
cd E:\VETAgent
python -m script.train_backbone_vetnet `
